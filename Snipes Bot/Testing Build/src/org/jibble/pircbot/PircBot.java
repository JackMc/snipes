/*
 * TODO: need GPL reference here
 * TODO: need reference to original project here
 */
...
/**
 * PircBot-PPF is a Java framework for writing IRC bots quickly and easily. 
 * This has been built up as a fork of PircBot from <a href="http://www.jibble.org">www.jibble.org</a>
 *  <p>
 ...
 *  <p>
 * Please visit the (original) PircBot homepage at
...
public abstract class PircBot implements ReplyConstants, PircBotLogger {
    ...
    public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if(_socketFactory == null) {
        	...
        	socket = _socketFactory.createSocket(hostname, port);
        }
     ...
     */
    protected void onConnect() {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_CONNENCT, new SnipesEventParams(this));}
     ...
     * performs no actions and may be overridden as required.
     */
    protected void onDisconnect() {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_DISCONNECT, new SnipesEventParams(this));}
     ...
     * @see ReplyConstants
     */
    protected void onServerResponse(int code, String response) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_SERVRESPONSE, new SnipesEventParams(this,Integer.toString(code),response));}
     ...
     */
    protected void onMessage(String channel, String sender, String login, String hostname, String message) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_CHANMSG, new SnipesEventParams(this,channel,sender,hostname,message));}
     ...
     */
    protected void onPrivateMessage(String sender, String login, String hostname, String message) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_PRIVATEMSG, new SnipesEventParams(this,sender,hostname,message));}
     ...
     */
    protected void onAction(String sender, String login, String hostname, String target, String action) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_ACTION, new SnipesEventParams(this,target,sender,hostname,action));}
     ...
     */
    protected void onNotice(String sourceNick, String sourceLogin, String sourceHostname, String target, String notice) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_NOTICE, new SnipesEventParams(this,sourceNick,sourceHostname,target,notice));}
     ...
     */
    protected void onJoin(String channel, String sender, String login, String hostname) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_NOTICE, new SnipesEventParams(this,channel,sender,hostname));}
     ...
     * performs no actions and may be overridden as required.
     * <p>
     * <b>Note: </b>this method is always called to maintain backwards compatibility with PircBot 1.4.4 implementations, the 
     * onPart(String channel, String sender, String login, String hostname, String reason) version is also called as well.
     *
     ...
     * 
     * @deprecated use {@link #onPart(String channel, String sender, String login, String hostname, String reason)} instead.
     */
    protected void onPart(String channel, String sender, String login, String hostname) {}

    /**
     * This method is called whenever someone (possibly us) parts a channel
     * which we are on.  The reason is included when used, an empty String otherwise. 
     *  <p>
     * The implementation of this method in the PircBot abstract class
     * performs no actions and may be overridden as required.
     * <p>
     * <b>Note: </b>onPart(String channel, String sender, String login, String hostname) is still called as well
     * to ensure backwards compatibility with PircBot 1.4.4 implementations.  It is strongly recommended to switch to
     * using this new method as the original onPart() is now deprecated and will be removed in a future version,
     *
     * @param channel The channel which somebody parted from.
     * @param sender The nick of the user who parted from the channel.
     * @param login The login of the user who parted from the channel.
     ...
     */
    protected void onPart(String channel, String sender, String login, String hostname, String reason) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_NOTICE, new SnipesEventParams(this,channel,sender,hostname,reason));}
     ...
     */
    protected void onNickChange(String oldNick, String login, String hostname, String newNick) {SnipesBot.sendEvent(SnipesEvent.SNIPES_IRC_NOTICE, new SnipesEventParams(this,oldNick,newNick,hostname));}
    ...
    /**
     * Sets the verbose mode. If verbose mode is set to true, then log entries will be printed to
     * the standard output (through the {@link #log(String)} method, which can be overridden). The
     * default value is false and will result in no output. For general development, we strongly
     * recommend setting the verbose mode to true.
     *
    ...
    /**
     * Gets the verbose mode. If verbose mode is set to true, then log entries will be printed to
     * the standard output (through the {@link #log(String)} method, which can be overridden). The
     * default value is false and will result in no output. For general development, we strongly
     * recommend setting the verbose mode to true.
     * 
     ...
     *
     * @param socketFactory The SocketFactory that should be used to make
     * connections.
     */
    ...
    /**
     * Returns the SocketFactory used by the PircBot so that you can alter
     * parameters.
     *
     * @return The actual SocketFactory used to make connections.
     */
    ...
    /**
     * Returns the User object for the specified nick in the specified 
     * channel.
     *  <p>
     * There are some important things to note about this method:-
     * <ul>
     *  <li>This method may not return the User if you call it
     *      before the complete nick list has arrived from the IRC server.
     *  </li>
     *  <li>This method will return immediately, as it does not require any
     *      interaction with the IRC server.
     ...
     * 
     * @return User object for the user if found, <code>null</code> if not found. 
     */
     ...
     * There are some important things to note about this method:-
     * <ul>
     *  <li>This method may not return the Object if you call it
     *      before the complete nick list has arrived from the IRC server.
     *  </li>
     *  <li>This method will return immediately, as it does not require any
     *      interaction with the IRC server.
     ...
     * @param sourceNick The nick of the user to find.
     * 
     * @return Object a custom object associated with this user, <code>null</code> if none found. 
     */
    ...
    /**
     * A convenience method to check that a user, by nick, is in a channel (any channel) 
     * with the bot.
     * 
     * Note that if you call this method immediately after joining a new channel, 
     * the new channel may not appear in this array as it is not possible to tell
     * if the join was successful until a response is received from the IRC server.
     * 
     * @param nick the nick to check for
     * @return true if the bot and user are in any channel together, false if not
     */
     ...
     *  
     * @param nick the nick to check for
     * @return A String array containing the names of all common channels that we
     *         and the user are in.
     */
    ...
    /**
     * Returns the single, highest (most powerful), mode that this user instance has.
     * 
    ...
    /**
     * Return the user prefixes known to be available on this server.  The highest (most powerful) 
     * mode being the leftmost.
     * 
    ...
    private SocketFactory _socketFactory = SocketFactory.getDefault();
    ...
    private Hashtable<String, String> _userPrefixes = new Hashtable<String, String>();
