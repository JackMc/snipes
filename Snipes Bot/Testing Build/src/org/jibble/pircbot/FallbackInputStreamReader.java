...
public class FallbackInputStreamReader extends Reader {
    ...
    private void fillBuffer() throws IOException {
        if (debug)
            logger.log(buf == null ? "FISR bp null bl null fl n/a" : ("bp " + buf.position() + " bl " + buf.limit() + " fl " + bufFillLevel));
        ...
        try {
            while (true) {
                ...
                if (r == -1) {
                    ...
                    buf.flip();
                    if (debug)
                        logger.log("FISR  -> 2 bp " + buf.position() + " bl " + buf.limit() + " fl " + bufFillLevel);
                    if (buf.remaining() == 0) {
                        buf = null;
                    }
                    return;
            ...
            buf.limit(0);
            if (debug)
                logger.log("FISR  -> " + e + " bp " + buf.position() + " bl " + buf.limit() + " fl " + bufFillLevel);
            throw e;
        }
    }
    ...
    private boolean checkLF(int start) {
        for (int i = start; i < buf.position(); ++i) {
            if (buf.get(i) == '\n') {
                ...
                buf.limit(i + 1);
                if (debug)
                    logger.log("FISR  -> 1 / " + start + " bp " + buf.position() + " bl " + buf.limit() + " fl " + bufFillLevel);
                return true;
            }
        }
        return false;
